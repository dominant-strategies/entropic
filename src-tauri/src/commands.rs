...
fn build_tools_markdown(capabilities: &[CapabilityState]) -> String {
    ...
    body.push_str(
        "\n## Office Files\n- Default to the AIO route for spreadsheet, document, and presentation work.\n- Use `entropic-office api inspect-aio /data/workspace/file.xlsx`, `...file.docx`, `...file.pptx`, or a supported text/PDF/image document path to inspect an existing file. Blank scaffolding is appropriate for editable formats like `.xlsx`, `.docx`, `.pptx`, and text documents.\n- Edit the returned `agent-interpretable-object` payload, then write it with `entropic-office api apply-aio <path>` using JSON on stdin.\n- Treat `object` as the canonical editable state. `analysis`, `annotations`, `projections`, and `artifacts` may exist as supporting context but should not silently replace `object`.\n- Spreadsheet AIO objects should use `kind: spreadsheet`, `object.kind: workbook`, and `object.worksheets`. Worksheets may also carry sparse `rows`, `columns`, `merged_ranges`, `freeze_pane`, and `filter_ref` metadata when layout semantics matter.\n- Document AIO objects should use `kind: document`, `object.kind: document`, and `object.blocks`. Blocks may include headings, paragraphs, lists, tables, quotes, code blocks, and inline semantics when the backend exposes them.\n- Native DOCX write now preserves headings, lists, tables, common inline marks, and hyperlinks; exact numbering templates, comments, and advanced layout are still partial.\n- Text-heavy document formats such as `.md`, `.markdown`, `.qmd`, `.txt`, `.html`, `.rst`, `.org`, `.adoc`, and `.tex` route through the Pandoc adapter when available.\n- PDF and supported image documents route through the Docling adapter for extraction, layout/provenance analysis, and backend projections. They are currently read-only.\n- Presentation AIO objects should use `kind: presentation`, `object.kind: deck`, and sparse `object.slides` entries with `layout_ref`, optional summary `title`/`body`, ordered `objects`, `frame`, optional `transition`/`timeline`, and optional `notes`.\n- Presentation objects may carry portable scene fields such as `shape_type`, `auto_shape`, `rotation`, `z_index`, `style.fill`, `style.stroke`, `crop`, chart `categories`/`series`, table `cells`, and image `image_ref`.\n- Presentation object edits can use `remove_object_ids`, `reorder_objects`, `prune_missing_objects`, style/rotation changes, image replacement, crop updates, and chart title/data replacement on existing charts.\n- Legacy `entropic-office spreadsheet new`, `spreadsheet todo`, `document new`, and `document lines` are compatibility helpers, not the default path.\n",
    );
