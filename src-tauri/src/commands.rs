...
fn build_tools_markdown(capabilities: &[CapabilityState]) -> String {
    ...
    body.push_str(
        "\n## Office Files\n- Default to the AIO route for spreadsheet, document, and presentation work.\n- Use `entropic-office api inspect-aio /data/workspace/file.xlsx`, `...file.docx`, or `...file.pptx` to inspect an existing file or scaffold a blank AIO object for a new path.\n- Edit the returned `agent-interpretable-object` payload, then write it with `entropic-office api apply-aio /data/workspace/file.xlsx`, `...file.docx`, or `...file.pptx` using JSON on stdin.\n- Spreadsheet AIO objects should use `kind: spreadsheet`, `object.kind: workbook`, and `object.worksheets`.\n- Document AIO objects should use `kind: document`, `object.kind: document`, and `object.blocks`.\n- Presentation AIO objects should use `kind: presentation`, `object.kind: deck`, and sparse `object.slides` entries with `layout_ref`, optional summary `title`/`body`, ordered `objects`, `frame`, optional `transition`/`timeline`, and optional `notes`.\n- Presentation object edits can use `remove_object_ids`, `reorder_objects`, `prune_missing_objects`, table `cells`, and image `image_ref` when needed.\n- Legacy `entropic-office spreadsheet new`, `spreadsheet todo`, `document new`, and `document lines` are compatibility helpers, not the default path.\n",
    );
